...
parser.add_argument('--num_iters', type=int, default=2e2, help='number of iterations of Lucas-Kanade')
...
#
    ...
    if ind in frame_eval:
        ...
        plt.savefig('CarTemplateCorr_frame_' + str(ind) +'threshold_'+ str(threshold) + 'iter_' + str(num_iters) +'.jpg')

    plt.show()
    plt.close()
np.save('carseqrects-wcrt.npy',np.asarray(rect_all))
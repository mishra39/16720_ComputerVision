    ...
    if v.ndim > 1:
        for i in range(v.shape[0]):
            for j in range(v.shape[1]):
                ...
                h1 = forward(x, params, 'layer1')
                probs = forward(h1, params, 'output',softmax)
                loss_sub, acc_sub = compute_loss_and_acc(y,probs)

                v_2dim = v[i,j]
                v[i,j] = v_2dim + eps
                h1 = forward(x, params, 'layer1')
                probs = forward(h1, params, 'output',softmax)
                loss_add, acc_add = compute_loss_and_acc(y,probs)

                grad[i,j] =  (loss_add - loss_sub) / (2*eps)
        ...
        for i in range(v.shape[0]):
            ...
            grad[i] =  (loss_add - loss_sub) / (2*eps)
